...
def create_custom_content(
    image: Image, content_type: str, stream: bool
) -> dict:
    attachments = []
...
def create_assistant_messages(
    images: List[Image], content_type: str, is_stream: bool
):
    for image in images:
        custom_content = create_custom_content(image, content_type, is_stream)
        yield {
            "role": "assistant",
            "content": "",
...
async def chat_completion(
    *,
    ...
    messages = list(
        create_assistant_messages(images, image_content_type, is_stream)
    )
