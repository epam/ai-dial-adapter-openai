...
|SSE_HEARTBEAT_INTERVAL||When defined, the adapter inserts ping comments into the streaming chat completions response every that many seconds. Useful if the upstream doesn't return response during streaming for a very long time and therefore the request to adapter fails somewhere because of a read-timeout error.|
