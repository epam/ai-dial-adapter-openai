...
> [!IMPORTANT]
> The adapter only proxies headers listed in `VLLM_HEADERS_TO_PROXY` for deployments registered in `VLLM_CHAT_COMPLETIONS_API_DEPLOYMENTS`. No additional headers are proxied for other (Azure/OpenAI/etc.) deployments.
...
|VLLM_HEADERS_TO_PROXY|``|Comma-separated list of HTTP header names to proxy **only** for vLLM deployments (those registered in `VLLM_CHAT_COMPLETIONS_API_DEPLOYMENTS`). Useful for passing a stable routing/caching marker to the vLLM cluster. Example: `x-conversation-id,x-tenant-id`|
