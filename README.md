...
To support such setups, configure `HEADERS_TO_PROXY` with a comma-separated list of incoming HTTP header names that should be forwarded to upstream model APIs.

Example:

- `HEADERS_TO_PROXY=x-conversation-id`

This increases the chance that related requests are routed to the same upstream instance and can benefit from upstream caching.

> [!IMPORTANT]
> The adapter proxies only headers listed in `HEADERS_TO_PROXY`.
...
|HEADERS_TO_PROXY|``|Comma-separated list of HTTP header names to proxy to upstream model APIs. Useful for passing a stable routing/caching marker. Example: `x-conversation-id,x-tenant-id`|
