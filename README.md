...
- Omit `key` and use AWS credentials from the environment (for example `AWS_PROFILE` and `AWS_DEFAULT_REGION`).
...
|AWS_DEFAULT_REGION||Default AWS region used by AWS SDK credential/config resolution for Bedrock authentication.|
