...
|FIX_STREAMING_ISSUES_IN_NEW_API_VERSIONS|False|When enabled, the response stream of chunks is guaranteed to be free of chunks with empty list of choices. Useful when a DIAL client doesn't support such chunks. Empty list of chunks are known to be generated by Azure OpenAI in at least two cases: (1) when the **Content filter** isn't disabled, Azure sends [prompt filter results](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/content-filter?tabs=warning%2Cuser-prompt%2Cpython-new#prompt-annotation-message) in the first chunk with empty list of choices (2) when `stream_options.include_usage` is enabled, the last stream chunk with the usage has empty list of choices|
