...
|VLLM_HEADERS_TO_PROXY|``|Comma-separated list of HTTP header names to proxy **only** for vLLM deployments (those registered in `VLLM_DEPLOYMENTS`). Useful for passing a stable routing/caching marker to the vLLM cluster. Example: `x-conversation-id,x-tenant-id`|
